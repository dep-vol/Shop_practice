    ...
    render () {
        ...
        let products = warehouse.products.map((product) => {
            ...
            if (!cartModel.inCart(product.id)) {
                btn = <button className="btn btn-success" onClick={() => cartModel.addCartItem(product.id)} style={{ marginRight: '5px' }}>
                        Add to cart
                      </button>
            }
            else {
                btn = <button className="btn btn-danger" onClick={() => {cartModel.deleteItem(product.id)}} style={{ marginRight: '5px' }}>
                        Remove from cart
                      </button>
            }
            return (
